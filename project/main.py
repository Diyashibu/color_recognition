...
def adjust_brightness(frame, factor=1.2):
    #convert to HSV toadjust brightness
...
def main():
    ...
    while True:
        ...
        if ret:
            #getting video ready for detection
            ...
            for color_name, bbox in detected_colors:
                ...
                cv2.putText(brightened_frame, color_name.capitalize(), (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2, cv2.LINE_AA)

            # Display the instruction to press ESC key to close in black color
            cv2.putText(brightened_frame, "Press ESC key to close", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2, cv2.LINE_AA)
